...
`PUT /employees/update/:id` registra o plugin `auth` e executa `checkIfEmployeeIsAdmin()` como primeira etapa do handler. Editar o cadastro de qualquer funcionário é operação administrativa; o alvo vem do `:id` na URL, não do JWT (diferente de `change-password`, que age sobre o próprio autenticado).
...
- **Verbo `PUT` x `PATCH`**: como o body é parcial (todos os campos opcionais), `PATCH` seria semanticamente mais alinhado a `activate`/`deactivate`. Mantido `PUT /update/:id` conforme implementado; revisitar se houver padronização futura do contrato.

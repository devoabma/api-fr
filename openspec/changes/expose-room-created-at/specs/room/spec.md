...
As salas MUST ser ordenadas por `createdAt` em ordem decrescente, e cada sala MUST devolver `id`, `name`, `standardTime`, `description`, `inactive` e `createdAt` (a data de cadastro da sala, para que o cliente possa exibi-la e reordenar localmente), além dos funcionários vinculados e dos `computers` (`id`, `macCode`, `number`, `description`, `inUse`, `maintenance`).
